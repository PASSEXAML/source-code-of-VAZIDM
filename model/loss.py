    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            with tf.compat.v1.Session() as sess:
                ...
                try:
                    sess.run(tf.debugging.assert_non_negative(self.theta))
                    print("self.theta has been calculated.")
                except tf.errors.InvalidArgumentError:
                    print("self.theta has not been calculated yet.")
            ...
            with tf.compat.v1.Session() as sess:
                sess.run(tf.compat.v1.global_variables_initializer())
                try:
                    sess.run(tf.debugging.assert_non_negative(theta))
                    print("theta has been calculated.")
                except tf.errors.InvalidArgumentError:
                    print("theta has not been calculated yet.")
            ...
            with tf.compat.v1.Session() as sess:
                sess.run(tf.compat.v1.global_variables_initializer())
                try:
                    sess.run(tf.debugging.assert_non_negative(t1))
                    print("t1 has been calculated.")
                except tf.errors.InvalidArgumentError:
                    print("t1 has not been calculated yet.")
